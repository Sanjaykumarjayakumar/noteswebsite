...
export default function NoteEditor() {
  ...
  const handleShare = async (includeFiles: boolean) => {
    try {
      if (!noteId) {
      ...
      const userdata = JSON.parse(localStorage.getItem("user") || "{}");
      await axios.put(`http://localhost:5000/api/notes/share/${noteId}`,
        {},
        {
          headers: {
            Authorization:
      ...
      const shareLink = `${window.location.origin}/share/${noteId}?includeFiles=${includeFiles}`;
      await navigator.clipboard.writeText(shareLink);
      setShared(true);
      setShowShareModal(false);
      toast.success(`Link copied${includeFiles ? " with files" : " without files"}`);
    }
    catch (err) {
